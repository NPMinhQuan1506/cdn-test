        ...
        if (myNode != null) {
            ...
            if (data && data.length > 0) {
                for (var i = 0; i < data.length; i++) {
                    _in++;
                    _dow=SysDate().FUTC(data[i].created_time).DOWText();
                    if (_in > 10 || i == 0 || _indow != _dow) {
                        myNode.insertAdjacentHTML("afterbegin",
                            ` <div class="d-flex my-2 text-dark align-items-center text-sm justify-content-center">
                                <div class="date text-xs">${SysDate().FUTC(data[i].created_time).DDowText()}</div>
                            </div>`);
                        _in = 0;
                        _indow = _dow;
                    }
                    ...
                    <div class="col-auto">
                        <div class="card card-plain">
                            <div class="card-body py-2 px-0 position-relative">
                                <div data-bs-toggle="tooltip" data-time="${SysDate().FUTC(item.created_time).TSPAN_NUM()}" title="${SysDate().FUTC(item.created_time).TimeText()}" class="con_itemsend text-dark mb-1">
                                    ${content}
                                    ${_isphone}
                                    ${_issent}
                                    ${_iserror}
    ...
    return new Promise((resolve) => {
        ...
        AjaxJWT(url = "/api/FacebookBridge/MarkResponse"
            ...
            , success = function (result) {
                ...
                if (data && data.length != 0) {
                    let time = SysDate().FUTC(data[0].uread_time).TSPAN_NUM();
