...
async function MSV_PreviewImg() {
    new Promise(() => {
        setTimeout(() => {
            $('body').on('click','.previewimg',function() {
                event.stopPropagation();
                if($(this).is('img')) {
                    let _prelink=$(this).attr('src');
                    if(_prelink!="") {
                        $('#MainPreview_Image img').attr('src',_prelink);
                        $('#MainPreview_Image').addClass('show');
                    }
...
function Master_Load_JS (callback) {
    try {
        ...
        js_require_notasync('/js/Datetime/Datetime_v2.js',false);
        js_require_notasync('/js/Datetime/function.js',false);
        ...
        //js_require_notasync('/js/Excel/Export.js',true);
        js_require_notasync('/js/Excel/lib/exceljs.js', true);
        js_require_notasync('/js/Excel/Export_v2.js', true);
        ...
        js_require('/js/comon/Library.js');
