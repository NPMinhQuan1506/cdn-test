...
let CVT_isVN = (author_get('UserLang') == 'vn' || author_get('UserLang') == '');
...
function Can_DrawDefault (objs, w, h, callback, unselect, goselect) {
    objs.each(function (i, obj) {
        ...
        var _nameTeeth=this.dataset.name;
        var updown = this.dataset.updown;
        var sign = this.dataset.sign;
        var imgex = this.dataset.imgex;
 
        var src=DendisR_DetectTeethSrc(_nameTeeth,updown, sign, imgex);
...
function DendisR_DetectTeethSrc(_nameTeeth,updown, sign, imgex) {
    if(updown!=undefined&&sign!=undefined) {
        //let _id=_nameTeeth.replace("Teeth","");
        //if(imgex!="") return imgex;
        return `/Canvas/${imgex}.svg`;
...
async function Dendis_GroupDiseaseRender (data, id) {
    new Promise((resolve) => {
        ...
        if (myNode != null) {
            ...
            for (let [key, value] of Object.entries(data)) {
                ...
                let picturecat = '', partname = '',disname='',teethname='';
                for (var i = 0; i < items.length; i++) {
                    let item = items[i];
                    if (i == 0) {
                        disname = CVT_isVN ? DiseaseList[item.Diseaseid].Name : (DiseaseList[item.Diseaseid].NameOther || DiseaseList[item.Diseaseid].Name);
                       
                        picturecat = DiseaseCatList[item.Type] ? ser_imageFeature_Disease + DiseaseCatList[item.Type].CatImg : Master_Default_Pic;
                    }
                    partname = CanTeethAreaName[item.Part];
                    if (partname != '') partname = `<span class="part pe-1">${CanTeethAreaName[item.Part]}</span>`;
                    teethname = teethname
                        + `<span>
                            <span class="px-1">${TeethCollkv[item.Teethid].TeethName}</span>
                            ${partname}
                            </span>`;
                }
                let tr = `
                        ...
                        <td>
                            <div class="d-flex">
                                ...
                                <div class="d-flex flex-column justify-content-center text-center">
                                    <h6 class="mb-0 text-sm fw-bold  text-dark">${teethname}</h6>
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    let tr = `
                        ...
                        <td>
                            <div class="d-flex">
                                <div class="avatar avatar-xs me-3">
                                    <img src="${picturecat}"  class="rounded-0 mt-1">
                                </div>
                        ...
                        <td>${CVT_isVN ? DiseaseList[item.Diseaseid].Name : (DiseaseList[item.Diseaseid].NameOther || DiseaseList[item.Diseaseid].Name)}</td>
