...
function addEmployeeConversation() {
    $("#DetailModal_Content").html('');
    $("#DetailModal_Content").load(`/Chatting/ConverEmployeeDetail?CustID=` + ser_MainCustomerID);
    $('#DetailModal').modal('show');
    return false;
}
...
function gen_ratingapp(custid) {
    let cona_keyCookie = 'ConnDevice';
    if (checkCookie(cona_keyCookie)) {
        let data = JSON.parse(getCookie(cona_keyCookie));
        if (data && data.length > 0) {
            let userdeviceto = data.map(e => e?.ID).join(',');
            let imageurl = ser_MainAvatarCustomer == '/default.png' ? '' : ser_MainAvatarCustomer;
            fcmmobile_sendmultidevice(userdeviceto, header = NameCustomer ?? "", content = Outlang["Danh_gia_ung_dung"], articleid = Number(custid), imageurl = imageurl)
            notiSuccess();
            return false;
        }
    }
    $("#DetailModal_Content").html('');
    $("#DetailModal_Content").load("/AppCustomer/Portal/ConnectingApp?ver=" + versionofWebApplication);
    $('#DetailModal').modal('show');
}
