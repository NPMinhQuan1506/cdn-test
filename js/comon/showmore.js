...
(function ($) {
    ...
    $.fn.TableExpandColumn = function (param) {
            ...
            if ($table.length > 0 && $itemsToogle.length > 0) {
                if ($table[0] != undefined) {
                    ...
                    for (let _i = 0; _i < $itemsToogle.length; _i++) {
                        ...
                        if (_savedsh[_dataname] != undefined && $itemsToogle[_i].checked != _savedsh[_dataname]) {
                            $itemsToogle[_i].checked = _savedsh[_dataname];
                        }
                    }
        ...
        function TableExpandColumn_Action() {
            ...
            if ($table.length > 0 && $itemsToogle.length > 0) {
                ...
                for (let _i = 0; _i < $itemsToogle.length; _i++) {
                    ...
                    if (_freezeColumn == 0) {
                        let _datafreeze = $itemsToogle[_i].dataset.freeze;
                        if (_datafreeze != undefined) {
                            _freezeColumn = _datafreeze;
                            TableExpandColumn_Freeze($table.attr('id'),$itemsToogle[_i].checked, _freezeColumn);
                        }
                    }
            
                    let $th = $table.find("thead").find('th[data-name="' + _dataname + '"]');
                    let $td = $table.find("tbody").find('td[data-name="' + _dataname + '"]');
                    let $tfoot = $table.find("tfoot").find('td[data-name="' + _dataname + '"]');
                    ...
                    if ($tfoot.length > 0) {
                        if ($itemsToogle[_i].checked) $tfoot.show();
                        else $tfoot.hide();
                    }
        ...
        function TableExpandColumn_Event() {
            let $itemsToogle = $(settings.shtoogle);
            $itemsToogle.change(function (event) {
                let $table = $(settings.Table);
                if ($table.length > 0) {
                    let _spename = ($table.attr('data-name') != undefined && $table.attr('data-name') != "") ? $table.attr('data-name') : $table[0].id;

                    let _dataname = $(this).attr('data-name');
                    let _freeze = $(this).attr('data-freeze');
                    let $th = $table.find("thead").find('th[data-name="' + _dataname + '"]');
                    ...
                    if (_freeze != undefined && _freeze != "") {
                        TableExpandColumn_Freeze($table.attr('id'), $(this).is(":checked"), _freeze);
                    }
                    if ($th.length > 0) {
        ...
        function TableExpandColumn_Freeze(tableId,freeze,number) {
            
            let styleId = `stylefreeze_${tableId}`;
            if (freeze) {
                if ($(`#${styleId}`).length === 0) {
                    let cssContent = "";
                    let totalWidth=0;
                    ...
                    for (let i = 1; i <= number; i++) {
                        ...
                        #${tableId} thead tr:not(.d-none):first-child th:nth-child(${i}) { 
                            position: sticky!important;
                            left: ${totalWidth}px;
                            z-index:1;
                            background-color: var(--color--table--head--freeze) ;
                            ${i==number? `border-right:3px dashed #fbcf33 !important;` :``}
                            box-shadow: inset 0 -1px 0 #fbcf33, 
                                inset -1px 0 0 #e9ecef, 
                                inset 0 1px 0 #e9ecef,   
                                inset 1px 0 0 #e9ecef;
                        }
                        }\n`;
                        ...
                        cssContent+=`
                        @media only screen and (min-width: 767px) {
                        #${tableId} tbody td:not(.d-none):nth-child(${i}) { 
                            background-color: var(--color--table--body--freeze);
                            position: sticky!important;
                            z-index:1;
                            left: ${totalWidth}px;
                            ${i==number? `border-right:3px dashed #fbcf33 !important;`:``}
                            box-shadow: inset -1px 0 0 #e9ecef, /* cạnh trái */
                                inset 1px 0 0 #e9ecef;  /* cạnh phải */
                        }
                        }\n`; 
                        let columnWidth=$(`#${tableId} thead tr:first-child th:not(.d-none):nth-child(${i})`).width() ?? 0;
