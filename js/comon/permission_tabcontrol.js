...
function Checking_RebuildHeader_Permission (dataHeader) {
    try { 
        let objHeader = {};
        for ([key, value] of Object.entries(dataHeader)) {
            let isAllow = true;
            if (value?.dataNamePer != undefined && value?.dataNamePer != '') {
                isAllow =  Checking_DataTab_IS_Allow(PermissionTable_TabControl, value?.dataNamePer) == 1;
            }
            let isShow = value?.isShow ?? true;
...
function secret_phone (phone) {
    let value = "*******";
    phone = removeHTML(phone).trim();
    value = value + phone.substring(phone.length - Master_SecretPhoneLength, phone.length)
    return value;
}
