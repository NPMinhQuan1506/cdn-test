...
function Load_Combo(dt, id, isRequire, allItem) {
    ...
    let outside = $('#' + id).is(':combo_offwscreen');
    if (outside) $('#' + id).parent().addClass('rightward');

    //$('#' + id).onClassChange((el, newClass) => {
    //    console.log(id)
    //    console.log(newClass)
    //    if (newClass.includes("animating") && !newClass.includes("rightward")) {
            
    //        let outside = $('#' + el.id).is(':combo_offwscreen');
    //        if (outside) $('#' + el.id).parent().addClass('rightward');
    //    }
    //});
...
function OnCombo_FillOption(id,data,isRequire) {
    try {
        ...
        else $(parent).find(".search").removeAttr('readonly');
...
jQuery.expr.filters.combo_offwscreen = function (el) {


    if (!$('#' + el.id).is(":visible")) {
        var rect = $('#' + el.id).parent()[0].getBoundingClientRect();
        return ((rect.x + 250) > window.innerWidth);
    }
    else return false;
