...
function Tab_Detail_Card_Changing_ByService () {
    ...
    let servicetype_id = 0;
    let service_id = CurrentService_Choose;
    let _obj_ser = data_service_root.filter(word => word["ID"] == service_id) ;
    if (_obj_ser != undefined && _obj_ser.length == 1) {
        servicetype_id = _obj_ser[0].CatID.toString();
    }
    if ($('#IsCheckUsing')[0].checked == false || service_id == 0) {
        ...
        let servicetype_string = ',' + servicetype_id + ',';
        let service_string = ',' + service_id + ',';
        $(".checkCard").attr("disabled", true);
        $(".card_detail_tab").addClass("card_hidden");
        for ([key, value] of Object.entries(data_tab_card)) {
            let servicetyperange = ',' + value.ServiceTypeRange + ',';
            let servicerange = ',' + value.ServiceRange + ',';
            if (servicerange.includes(service_string) || servicetyperange.includes(servicetype_string) || value.AllService == 1) {
