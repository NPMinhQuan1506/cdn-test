...
function OnChange_Services(teeth_in_update) {
    ...
    if(tab_changing_flag==1||teeth_in_update!=undefined) {
        ...
        if (service_id != 0) {
            $('#tab_wareqtity').val('');
            $('#tab_warecheck').removeClass('success').removeClass('fail');
            ...
            if (Service[0]?.IsInstallment == 0) $('.Tabins_Sup').addClass('disabled');
            else $('.Tabins_Sup').removeClass('disabled');
            if(Service[0]?.VAT!=0) {
         
                $("#vat_per").dropdown("refresh");
                $("#vat_per").dropdown("set selected",Service[0]?.VAT);
            }
            let dayexpired=Number(Service[0]?.ExpiredDay);
            if(dayexpired>0) {
                tabExpiredDay_Set(dayexpired);
                $('#tab_expiredtext').html(`${dayexpired} `+Outlang["Ngay"]);
            }
            else {
                tabExpiredDay_Clear();
                $('#tab_expiredtext').html(``);
            }
...
function Tab_Detail_Format_Combo() {
    if (isUsingTabComboService == 1 && CurrentTabCombo_IsNotChangeAllow == 1) {
        $(".rowseletedcombo").find(".field").addClass("disabledfield")
    }
    else {
        $(".rowseletedcombo").find(".field").removeClass("disabledfield")
    }
}
...
function Tab_Detail_Teeth_Show_Hide(ServiceTab, teeth_in_update) {
    ...
    if (sys_DentistCosmetic == 1 && ServiceTab != 0) {
        ...
        if ((currentUnitTypeTab == 'teeth' || currentUnitTypeTab == 'eachtooth') && data_tab_current_is_product == 0 && teeth_in_update == undefined) {
            ...
            let TeethDenture = (currentUnitTypeTab == "eachtooth") ? 1 : 0;
...
function Render_Current_Selected_Service_Tab(key, value, id) {
    ...
    if (myNode != null) {
        let discount = '', insurance = '', install = '', cardused, isfree = '', ischeck = '', vat = '', isCheckInventories = '';
        ...
        if (value.IsCheckStock == 1 && value.IsChoose == 1) {
            let clsInv = value.IsValidStock == 1 ? `text-success` : `text-danger`;
            isCheckInventories = `<span class="${clsInv} fw-normal ms-2">${Outlang["So_luong_ton_kho"]}: ${value.NumInv}</span>`;
        }
        ...
        let tr =
            ...
            + discount + isfree + cardused + insurance + install + vat + isCheckInventories
            + '</div>'
            + '</div></div>'
            + '<div class="d-flex">'
...
function ServiceTem_Load(obj) {
    ...
    final_insamount = Number(obj.Installment_Fee);
    ...
    if (obj.IsInsurance == 1) {
        ...
        finalinsur_Amount = Number(obj.InsurAmount);
    ...
    if (obj.IsInstallment == 1) {
        ...
        $("#Tabins_Sup").dropdown("set selected", obj.Installment_SupCode);
        $("#Tabins_Term").dropdown("refresh");
        $("#Tabins_Term").dropdown("set selected",obj.Installment_Term);
        $("#Tabins_Frequency").dropdown("refresh");
        $("#Tabins_Frequency").dropdown("set selected",obj.Installment_Frequency);

        

        $('#Tabins_Amount').val(obj.Installment_Fee);
    }
...
function Genre_PaymentReview() {
    ...
    stringcontent = '<div class="d-flex justify-content-between">'
        ...
        + Genre_PaymentReview_Detail(finalinsur_Name + " " + Outlang["Sys_bao_lanh"], 0, finalinsur_Amount)
        + Genre_PaymentReview_installment(final_insamount)
...
//    }
//}
function Genre_PaymentReview_installment(__val) {

    if (__val == '' || Number(__val) == 0) return '';
    else {
        return '<div class="d-flex justify-content-between">'
            + '<span class="mt-1 text-sm"> ' + Outlang["Sys_phi_tra_gop"] + '</span>'
            + '<span class="text-dark ms-4">' + formatNumber(__val) + '</span>'
            + '</div>'
...
// #endregion
//#region // Expired date
function tabExpiredDay_Clear() {
    Sys_Picker("#tabExpiredDay",false,{
        minDate: new Date()
    });
 
}
function tabExpiredDay_Set(numday) {
    let date=(new Date()).addDays(numday);
    Sys_Picker("#tabExpiredDay",false,{
        minDate: new Date(),
        defaultDate: date
    });
 ...
 //#endregion
//#region // Check ware
function tabWare_Check(prodic, pronumber, prounitname, numexchange) {
 
    let _e={};
    _e.packageNumber="";
    _e.idProduct=prodic;
    _e.number = pronumber * numexchange;
    let dataProduct=[];
    dataProduct.push(_e);
    AjaxLoad(url="/Customer/Service/TabMulti/?handler=CheckQuantityRealTime"
        ,data={
            'data': JSON.stringify(dataProduct)
            ,'ware': data_WareExportID
        }
        ,async=true
        ,error=null
        ,success=function(result) {
            if(result!="0") {
                ...
                let _datapro=datas.Table;
                if (_datapro != undefined && _datapro.length == 1) {
                    $('#tab_wareqtity').val(`${_datapro[0].UnitName != prounitname
                        ? (`${_datapro[0].FinalNum / numexchange} ${prounitname}`)
                        : (`${_datapro[0].FinalNum} ${_datapro[0].UnitName}`)}`);
                    if (_datapro[0].FinalNum >= 0) {
                        $('#tab_warecheck').addClass('success').removeClass('fail');
                    }
                    else {
                        $('#tab_warecheck').addClass('fail').removeClass('success');
                    }
                }
            }
            else {
...
function SaveEachTab(continute) {
    ...
    let dateCreated=(ser_ChooseDateCustomer=="1")? Sys_PickerGet('dateCreatedExt'):new Date();
    let dateExpired = $('#tabExpiredDay').val() ? $('#tabExpiredDay').val() : '01-01-1900';
    ...
    if (_insurance != 0) {
        if (finalinsur_Amount < 0) {
            document.getElementById("textShowMessage").innerHTML = Outlang["Kiem_tra_du_lieu"] + ' - ' + Outlang["Bao_hiem"];
        }
    }
    ...
    let IsCheckStock = $('#tab_warecheck')?.hasClass('fail') || $('#tab_warecheck')?.hasClass('success') ? 1 : 0;
    let IsValidStock = $('#tab_warecheck')?.hasClass('fail') ? 0 : 1;
    let NumInv = $('#tab_wareqtity')?.val() ?? "";

    if (data_WareValidSell == 1 && IsChoose == 1 && IsCheckStock == 1 && IsValidStock == 0) {
        document.getElementById("textShowMessage").innerHTML = Outlang["Kiem_tra_so_luong_ton_kho"] ?? "Kiểm tra số lượng tồn kho";
    }
    ...
    if (document.getElementById("textShowMessage").innerHTML == "") {
        ...
        let dtSerCurr = data_service_root.filter(word => word["ID"] == ServiceTab) ?? [];
        var elementItem = {};
        elementItem.ServiceID = ServiceTab;
        if (dtSerCurr && dtSerCurr.length > 0) {
            elementItem.ServiceName = dtSerCurr[0]?.Name ?? "";
        } else elementItem.ServiceName = $('#tabservice_input').val();
        ...
        elementItem.Discount_CamVoucher_DetailID = Number($("#DiscountCamVoucherID").dropdown('get value')) ? Number($("#DiscountCamVoucherID").dropdown('get value')) : 0;
        ...
        elementItem.dateCreated = SysDate().FUTC(dateCreated).UTCText();
        elementItem.dateExpired = dateExpired;
        ...
        let ins_Term = Number($('#Tabins_Term').dropdown('get value')) ? Number($('#Tabins_Term').dropdown('get value')) : 0;
        let ins_Frequency = Number($('#Tabins_Frequency').dropdown('get value')) ? Number($('#Tabins_Frequency').dropdown('get value')) : 1;



        if (ins_Sup != '' && ins_Term != 0 && ins_Frequency != 0) {
            elementItem.IsInstallment = 1;
            elementItem.Installment_Fee = $('#Tabins_Amount').val() ? $('#Tabins_Amount').val() : 0;
            elementItem.Installment_Term = ins_Term;
            elementItem.Installment_Frequency = ins_Frequency;
            elementItem.Installment_SupCode = ins_Sup;
        }
        else {
            elementItem.IsInstallment = 0;
            elementItem.Installment_Fee = 0;
            elementItem.Installment_Term = 0;
            elementItem.Installment_Frequency = 0;
            elementItem.Installment_SupCode = '';
        }
        ...
        elementItem.InsurAmount = finalinsur_Amount;
        if (_insurance != 0) {
        ...
        if (sys_TreatManualIndex == 1) {
            ...
            if (!isNaN(_timetotreat) && _timetotreat != "" && _timetotreat > 0) {
                elementItem.TimeToTreat = Math.floor(_timetotreat);
            }
        ...
        //Check inventory
        elementItem.IsCheckStock = IsCheckStock;
        elementItem.IsValidStock = IsValidStock;
        elementItem.NumInv = NumInv;
...
function TM_CheckDispro (num, discountid) {
    ...
    if (TabCurrentID == 0) {
        ...
        if (_da != undefined && _da.length == 1) {
            let _it = _da[0];
            if (_it.Numberuse == 1) {
                _numf = Number(_it.Numberfrom);
                _numt = Number(_it.Numberto);
    ...
    if (!res) {
        $('#tab_discountnoti').removeClass('d-none');
        $('#tab_discountnoti').attr('data-bs-original-title', `${Outlang["So_luong_cho_phep"]} : ${_numf.toString()} -${_numt.toString()}`  );
        ToolPopper();
    }
    else {
        $('#tab_discountnoti').addClass('d-none');
    }
    return res;
}
...
function Executing_Save_Current_Choosing_Service() {
    ...
    let dateBegin=SysDate().FUTC(Sys_PickerGet('#date-begin')).UTCDateText() ;// ($('#date-begin').length) ? $('#date-begin').val() : '';
    let dateEnd=SysDate().FUTC(Sys_PickerGet('#date-end')).UTCDateText()   ;//($('#date-end').length) ? $('#date-end').val() : '';
    let comboID = (isUsingTabComboService == 1 ? CurrentServiceTabCombo_Choose : 0)
    var objexetab = data;
    AjaxJWT(url = "/api/Customer/Tab/ExecuteData"
        , data = JSON.stringify({
        ...
        , success = function (result) {
            if (result != "0") {
                ...
                if (data && data.length > 0) {
                    let _RESULT = data[0]?.RESULT ?? 0;
                    let ProTabTokenID = data[0]?.ProTabTokenID ?? "";
                    let IsExport = data[0]?.IsExport ?? "";
                    ...
                    if (_RESULT == "2") {
                        ...
                        notiError(Outlang["Ngay_xu_ly_phai_lon_hon_ngay_chot_kho_gan_nhat"] ?? "Ngày xử lý phải lớn hơn ngày chốt kho gần nhất");
                    }
                    else if (_RESULT == "0") {
                        notiError_SW();                   
                    }
                        ...
                        if(IsExport==1) {
                            ...
                            if (typeof Maincust_StockOut == "function") {
                                Maincust_StockOut({
                                    TokenID: ProTabTokenID, Type: "ser", IsActionAuto: 1,
                                    fnSaveComplete: function (re) {
                                        if (typeof re != "undefined" && re?.length > 0) {
                                            let promises = [];
                                            let type = syn_accWareExportTS;
                                            let ExportID = 0;
                                            for (let i = 0; i < re.length; i++) {
                                                let itemRe = re[i];
                                                let _exportID = re[0]?.ExportID ?? 0;
                                                let _WareIsExport = re[0]?.IsExport ?? 0;
                                                if (_exportID > 0 && _exportID != ExportID && _WareIsExport == 1) {
                                                    ExportID = _exportID; 
                                                    if (syn_Accountbrand != undefined && syn_Accountbrand != "" && syn_accWareExportTS != 0 ) {
                                                        promises.push(
                                                            APIAccount_Excute(currentID = ExportID, type = type, action = APIAccount_WareSave ?? "waresave"
                                                                , beforefunc = function () { }
                                        ...
                                        if (typeof TabList_Service_Loading_After_Modified_Service === 'function') {
                                            TabList_Service_Loading_After_Modified_Service(current_adding_new_plan);
                                        }
                                    }
                                });
