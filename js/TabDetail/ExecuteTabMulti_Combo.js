    ...
    return new Promise((resolve, reject) => {
        setTimeout(
            () => {
                ...
                let searchtext=$('#tabservice_combo_input').val();
...
function Event_SearchRowComboService_Div () {
    ...
    $(".rowcombotabservice").on("click", function () {
        ...
        if (data != undefined && data != null && data.length == 1) {
            ...
            CurrentTabCombo_IsNotChangeAllow = data[0]?.IsNotChangeAllow ?? 0;
        }
...
function Event_ClickDetailRowComboService_Div () {
    $(".rowCombotabservice").on("click",function() {
        ...
        let _numquan = $(this).attr('data-quantity');
       
        if (Number(_numquan) != 0 && sys_DentistCosmetic == 0) {
            $('#Tab_Service_Quantity').val(_numquan);
        }
        if(Number(_numtimes)!=0&&sys_DentistCosmetic==0) {
            $('#service_timetotreat').val(Number(_numtimes)*Number(_numquan));
        }
