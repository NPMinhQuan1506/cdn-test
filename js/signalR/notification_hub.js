...
connection.on("Send_Message_AppNote",function(jsonString) {

    if(typeof Appnote_Hub=='function') {
        Appnote_Hub(jsonString);
    }
});
connection.on("SendRating_Success", function (jsonString) {

    if (typeof Noti_RatingApp == 'function') {
        Noti_RatingApp(jsonString);
    }
});
connection.on("SendBooking_Success",function(jsonString) {
   
    if(typeof Noti_BookingOnline=='function') {
        Noti_BookingOnline(jsonString);
    }
});
connection.on("SendProfile_Success",function(jsonString ) {
        
    if(typeof Noti_ProfileOnline=='function') {
        Noti_ProfileOnline(jsonString );
    }
});
connection.on("SendPayment_Track",function(jsonString) {

    if(typeof Noti_PaymentTrack=='function') {
        Noti_PaymentTrack(jsonString);
    }
});
...
// GD Login success
connection.on("GDSend_Message_To_SuccessLogin",function(jsonString) {
    if(typeof GD_LoginSuccessFromCloud==='function') {
        GD_LoginSuccessFromCloud(jsonString);
    }
});
