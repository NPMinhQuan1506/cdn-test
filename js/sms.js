...
function SMSSYS_ActionSend (_issign, _phone, _content, _amount, _brandname,_znssms
    , _idsms
...
//    catch (ex) { }
//}
function SMSSYS_MultiAction (_datamess, _brandname, iszns = 0, _successfunc, _errorfunc) {
    try {
        AjaxJWT(url = "/api/SMS/SendMulti" + "?ver=" + RandomNumber()
            , data = JSON.stringify({
                'SmsItem': _datamess,
                'BrandName': _brandname,
                'IsZns': iszns,
    ...
    return new Promise((resolve, reject) => {
        AjaxJWT(url = "/api/SMS/afterChange"
            , data = JSON.stringify({
                ...
                , 'cardid': _cardid
                , 'smscurbrand': SMSCurBrandName != undefined ? SMSCurBrandName.toLowerCase() : ""
