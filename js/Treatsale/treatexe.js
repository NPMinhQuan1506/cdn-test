            ...
            if (result != "0") {
                ...
                $('#met_package_' + id).unbind('change').change(function () {
                    ...
                    if (val != undefined && val != 0) {
                        ...
                        $('#met_packageExpired_' + id).html(SysDate().FUTC(Met_DataPackage[val].DateExpired).DateText());
...
function Met_Renproduct (randomNumber) {
    let resulf =
        `<div class="ui fluid search border-0 selection dropdown productExport form-control" id="productExport_${randomNumber}">
                <input type="hidden" />
