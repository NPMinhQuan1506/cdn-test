            ...
            if (result != "0") {
                ...
                $('#DSD_package_' + id).unbind('change').change(function () {
                    ...
                    if (val != undefined && val != 0) {
                        ...
                        $('#DSD_packageExpired_' + id).html(SysDate().FUTC(DSD_DataPackage[val].DateExpired).DateText());
...
function DSD_Renproduct (randomNumber) {
    let resulf =
        `<div class="ui fluid search border-0 selection dropdown productExport form-control" id="DSD_product_${randomNumber}">
                <input type="hidden" />
