...
function Payment_Detail_Render_Service(ServiceCatName, TeethChoosing, TeethType, createdby, created) {
    ...
    let createdinfo = '<p class="text-sm text-dark d-inline ms-2">' + name + ' , ' + SysDate().FUTC(created).DateText() + '</p>';
...
async function Payment_Detail_Render(data, id, id2,id3) {

    new Promise((resolove) => {
        var myNode = document.getElementById(id);
        var myNode_TreatInDay = document.getElementById(id2);
        var myNode_serInDay = document.getElementById(id3);
        let isRender = 0;
        if (myNode != null) {
            isRender = 1;
            myNode.innerHTML = "";
        }
        if (myNode_TreatInDay != null) {
            isRender = 1;
            myNode_TreatInDay.innerHTML = "";
        }
        if (myNode_serInDay != null) {
            isRender = 1;
            myNode_serInDay.innerHTML = "";
        }
        if (isRender == 1) {
            let isTreatInday = 0,isSerInday = 0, isPaymentAll = 0;
            if (data && data.length > 0) {
                for (let i = 0; i < data.length; i++) {
                    ...
                    let tr = `
                    <div id="rowPaymentDetail_${item.TabID}" class="rowPaymentDetail ${isFree}" >
                        <div class="d-flex">
                            <div class="optarea mt-n0 ms-n3 me-2 form-check opacity-0">
                                    <input data-id=${Number(item.TabID)} data-left=${Number(item.TabLeft)} class="form-check-input opt" type="checkbox" >
                            </div>
                            <div class="my-auto">
                                <div class="h-100">
                    ...
                    `
                    if (item.TreatInDay == 1 && item.SerInDay == 0) {
                        if (myNode_TreatInDay != null) {
                    ...
                    if (item.SerInDay != 0) {
                        if (myNode_serInDay != null) {
                            myNode_serInDay.insertAdjacentHTML('beforeend', tr);
                            isSerInday = 1;
                        }
                    }
                    if (item.TreatInDay == 0 && item.SerInDay == 0) {
                        if (myNode != null) {
            ...
            if (isSerInday == 1) $("#title_ser_inday").fadeIn(100);
            else $("#title_ser_inday").remove();
...
function Payment_Detail_Event() {
    ...
    if (ser_TotalDeposit < 0 || (ser_TotalDeposit == 0 && ser_TotalVirtual <= 0)) {
        $("#deposit_info").hide();
    ...
    if(ser_TotalVirtual<=0) {
        $("#paydes_virtualarea").hide();
    }
    else {
        $("#paydes_virtualarea").show();
    }
...
//#region // OPtion payment
function PDC_OptionOpen( ) {                  
    if($('#pd_optionselect').hasClass('show')) {
        PDC_OptionEnd();
    }
    else {
        PDC_OptionStart();
...
function PDC_OptionStart() {
    $('#LPS_ListPayment').addClass('option');
    $('#pd_optionselect').addClass('show');
    $('#pdc_optionbtn').addClass('active');
}
function PDC_OptionEnd() {
    $('#LPS_ListPayment').removeClass('option');
    $('#pd_optionselect').removeClass('show');
    $('#pdc_optionbtn').removeClass('active');
    $('#pcd_optAmount').removeClass('error');
    $('#pcd_optRest').html('');
    
}
function PDC_OptionCount() {
    let roundUnit = Number(sys_PayOptNumDevide) ?? 1000;
    let opt=$('#LPS_ListPayment .opt[type="checkbox"]:checked');
    let _array=[];
    $('#pcd_optAmount').removeClass('error');
    $('#pcd_optRest').html('');
    if(opt.length>0) {
        let total=0;
        opt.each(function() {
            let dataLeft=parseFloat($(this).data('left'));
            let dataId=parseFloat($(this).data('id'));
            if(!isNaN(dataLeft)) {
                total+=Number(dataLeft);
                let e={};
                e.id=dataId;
                e.amount=Number(dataLeft);
                e.allocatedAmount=0;
                _array.push(e);
            }
        });

        let _val=parseFloat($('#pcd_optAmount').val());
        if(!isNaN(_val)&&_val>0&&_val<=total&&total>0) {
            ...
            for(let ii=0;ii<_array.length;ii++) {
                ...
                if(ii===_array.length-1) {
                    let maxAllocation=_array[ii].amount;
                    allocation=Math.min(remainder,maxAllocation);
                } else {
                    let maxAllocation=_array[ii].amount;
                    allocation=Math.min((_array[ii].amount/total)*_val,maxAllocation);
                }
                allocation=Math.floor(allocation/roundUnit)*roundUnit;
                _array[ii].allocatedAmount=allocation;
                remainder-=allocation;
            }
            if(remainder>0) $('#pcd_optRest').html(`${Outlang["Con_lai"]} : ${formatNumber(remainder)}`);
            for(let ii=0;ii<_array.length;ii++) {
                if(_array[ii].allocatedAmount!=0) {
                    $('#payCurrent_'+_array[ii].id).val(_array[ii].allocatedAmount);
                }
            }
 
        } else {
            $('#pcd_optAmount').addClass('error');
        }
    } else {
        notiWarning(Outlang["Kiem_tra_du_lieu"]+': '+Outlang["Tien_thanh_toan"]);
    }
}
