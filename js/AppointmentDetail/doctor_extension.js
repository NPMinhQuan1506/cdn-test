    ...
    new Promise((resolve, reject) => {
        setTimeout(function (e) {
            if ($('#app_detail_ex_timeline').length) {
                var date = new Date();
                let date_begin=SysDate().FUTC(date).UTC_Date();
                ...
                $("#doctor_sheduler_in_one_day .ruler").height(70);
                AD_TimeLineHover();
            }
        }, 400);
    })
...
async function AD_TimeLineLoad() {
    $("#button_save_appointment").prop("disabled", true); 
    new Promise((resolve, reject) => {
        setTimeout(function (e) {
            $("#apptotal_branch").html($('#App_Detail_Branch_ID').dropdown('get text'));
            $("#apptotal_date").html($('#App_Detail_Date_From').val());


            $("#textShowMessage").html('');
            let date = $('#App_Detail_Date_From').val() + ' ' + $('#App_Detail_DateFrom_Time').val();
            let date_now = SysDate().FNOW().UTC_Date();
            ...
            if (branchID != 0 && doctorID != 0 && $('#app_detail_ex_timeline').length != 0) {
                AD_TimeLineClear();
                $("#appdoc_timearea").removeClass('d-none');
                $("#appdoc_timedocname").html($('#App_Detail_Doctor_ID').dropdown('get text'));
                $("#appdoc_timedate").html($('#App_Detail_Date_From').val());
                $("#appdoc_timelabelarea").removeClass('d-none');
                AjaxLoad(url = "/Appointment/AppointmentDetail/?handler=LoaData_Extension_Doctor"
                    ...
                    , success = function (result) {
                        if (result != "0") {
                            ...
                            let _data_empwork = _dataex.EmpWork;
                            ...
                            let is_out_time_line = 0, is_none_time_line = 0, is_other_branch = 0;
                            sys_AppDoctorNotWorkSchedule = sys_AppDoctorNotWorkSchedule ?? 0;
                            if (sys_AppDoctorNotWorkSchedule == 0) { 
                                if (_data_work && _data_work.length != 0) {
                                    for (_ii = 0; _ii < _data_work.length; _ii++) {
                                        if (Number(_data_work[_ii].Off) == 0) {
                                            ...
                                            if (Number(_data_work[_ii].IsOtherBranch) == 1) {
                                                is_other_branch = 1;
                                            } else {
                                                is_none_time_line = 1;
                                            }
                                    ...
                                    if (_data_empwork && _data_empwork.length != 0) {
                                        let { IsWorkScheduler } = _data_empwork[0];
                                        if (IsWorkScheduler == 1) {
                                            is_none_time_line = 1;
                                        }
                                    }
                                }
                            ...
                            if (is_other_branch == 1) {
                                $("#textShowMessage").html(Outlang["Sys_bac_si_khong_co_lich_lam_viec_o_chi_nhanh_nay"] ?? 'Bác sĩ không có lịch làm việc ở chi nhánh này'); 
                            }
                            if(is_out_time_line != 1 && is_none_time_line != 1 && is_other_branch != 1) $("#button_save_appointment").prop("disabled", false);
                ...
                $("#appdoc_timedocname").html('');
                $("#appdoc_timedate").html('');
                $("#appdoc_timearea").addClass('d-none');
                $("#appdoc_timelabelarea").addClass('d-none');
