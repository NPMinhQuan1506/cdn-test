...
function AD_RenderInfo (dataBranch, dataCustomer) {
    ...
    if (dataCustomer && dataCustomer.length > 0) {
        $('#appnext_area').removeClass('d-none');
        $("#appnext_timedate").html(dataCustomer[0].Date_From);
        $("#appnext_custname").html(dataCustomer[0].CustName);
        $("#appnext_branchname").html(dataCustomer[0].BranchName);

    }
    else {
        $('#appnext_area').addClass('d-none');
...
function AD_SaveExecutedLoad () {
    ...
    if (typeof mns_LoadData === 'function') {
        notiSuccess();
        mns_LoadData();
    }
    ...
    else if (typeof LoadAppointmentListAjax === 'function') {
        LoadAppointmentListAjax((ser_CurrentAppointmentID ?? 0), 1);
    }
    else {
        location.reload();
    }
...
//#endregion
//#region // ReTreat
function AD_RetreatShow(CustomerID, ScheduleID = 0) {
    $("#DetailModal_Content").html('');
    $("#DetailModal_Content").load("/Appointment/AppointmentReTreatUpdate"
